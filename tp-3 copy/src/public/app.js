function init() {
  ...
  function sendMessage() {
    ...
    if (!message) return; // Previene el envío de mensajes en blanco

    const chatKey = `chat_${chat.person}`; // Clave para localStorage
    ...
    messages.push({ message: encryptedMessage, fromSelf: true }); // Marcar como enviado
...
socket.on('chat_message', data => {
  // Verifica si el chat actual es el mismo que el chat del mensaje recibido
  if (!chat.current || chat.person !== data.person) {
    console.warn("Message received but no matching active chat.");
    return; 
  }

  // Verifica que el mensaje no venga del mismo cliente (para evitar duplicados)
...
socket.on('user_status', (data) => {
  const personElement = document.querySelector(`.person[data-chat="${data.person}"]`);
  if (personElement) {
    // Crear o actualizar el ícono de estado
    let statusIcon = personElement.querySelector('.status-icon');
    if (!statusIcon) {
      statusIcon = document.createElement('span');
      statusIcon.classList.add('status-icon');
      personElement.appendChild(statusIcon);
    }
    statusIcon.style.backgroundColor = data.status === 'online' ? 'green' : 'red';
  }
});
...
function loadChatHistory(person) {
  ...
  messages.forEach(msg => {
      ...
      // Agregar el mensaje, independientemente de si fue descifrado o no
