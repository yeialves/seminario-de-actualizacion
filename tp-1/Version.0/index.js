...
app.post('/register', (req, res) => {
    ...
    connection.query("SELECT id, password FROM User WHERE username = ?", [username], (selectError, selectResults) => {
        if (selectError) {
        ...
        if (selectResults.length > 0) {
            ...
            if (storedPassword === password) {
                associateUserWithGroups(userId, groups, res);
            } else {
                return res.status(401).json({ error: "Invalid username or password" });
            }
        } else {
            // Insertar un nuevo usuario si no existe
            connection.query("INSERT INTO User (username, password) VALUES (?, ?)", [username, password], (insertError, insertResults) => {
                if (insertError) {
...
// Función para asociar un usuario con grupos
function associateUserWithGroups(userId, groups, res) {
    const groupIds = groups.map(group => parseInt(group, 10)).filter(id => !isNaN(id));
    const userGroupValues = groupIds.map(groupId => [userId, groupId]);

    connection.query("INSERT IGNORE INTO User_Group (user_id, group_id) VALUES ?", [userGroupValues], (error, results) => {
        if (error) {
            console.error('Error associating user with groups:', error);
            return res.status(500).json({ error: "Internal server error during group association" });
        }
        res.status(201).json({ message: "Account created or updated successfully", id: userId });
    });
}
...
app.post('/user-actions', (req, res) => {
    ...
    connection.query(sql, [username], (error, results) => {
        ...
        res.status(200).json(results.map(row => row.name));
    });
});
